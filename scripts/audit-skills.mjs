...
export function isValidHttpsSource(value) {
  try {
...
export function findLocalReferences(content) {
...
export function buildVerdictReason(
  entry,
  dimensions,
  age,
  verdict,
...
export async function auditSkills(root, asOf = new Date(), auditedAt = asOf) {
  ...
  for (const entry of registry.skills) {
    ...
    const localReferences = findLocalReferences(content);
    ...
    const routingExcludes = entry.routing?.excludes ?? [];
    const validRoutingExcludes = routingExcludes.length > 0 && routingExcludes.every((name) => {
      const target = registry.skills.find((candidate) => candidate.name === name);
      return target
    ...
    const dimensions = {
      triggerPrecision:
        ...
        + (validRoutingExcludes || entry.routing?.role === "reference" ? 3 : 0),
      actionability:
      ...
      sourceQuality:
        (Array.isArray(entry.sourceUrls) && entry.sourceUrls.length > 0 && entry.sourceUrls.every(isValidHttpsSource) ? 5 : 0)
        + (provenanceOwner ? 5 : 0)
      ...
      uniqueness:
        ...
        + ((entry.related ?? []).length > 0 || validRoutingExcludes || entry.routing?.role === "reference" ? 2 : 0),
    };
    const score = Object.values(dimensions).reduce((total, value) => total + value, 0);
    ...
    skills.push({
      ...
      verdict,
      dimensions,
      reason: buildVerdictReason(entry, dimensions, age, verdict, mergeTarget, overlapIntents),
      evidence: {
        lines,
